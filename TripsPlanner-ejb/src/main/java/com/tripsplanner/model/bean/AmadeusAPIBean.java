...
public class AmadeusAPIBean {
    ...
    public JSONObject getHotelsJson(Float lat, Float lon,  String departure_date, String return_date) throws Exception {
        String amadeus_token = getAmadeusToken();
...
//        Response response = client.newCall(request).execute();
//        String response_body = response.body().string();
        String requestUrl = "https://test.api.amadeus.com/v1/shopping/hotel-offers?latitude=" + lat +"&longitude=" + lon +"&radius=5&radiusUnit=KM&includeClosed=false&checkInDate=" + departure_date + "&checkOutDate=" + return_date +"&bestRateOnly=true&view=FULL"; //+"&page%5Blimit%5D=5"
        JSONObject jsonObj = runConnection(requestUrl, amadeus_token);

        return jsonObj;
    }
    
    public ArrayList<Hotel> getHotels(HashMap<String, Float> map,  String departure_date, String return_date) throws Exception {
        ...
        JSONObject json = getHotelsJson(lat, lon, dep_date, ret_date);
